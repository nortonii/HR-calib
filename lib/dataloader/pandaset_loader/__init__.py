...
# ── PandaSet Camera Loader ─────────────────────────────────────────────────────

PANDASET_CAMERA_NAMES = [
    "front_camera", "front_left_camera", "front_right_camera",
    "back_camera", "left_camera", "right_camera",
]


def _load_pandaset_intrinsics(cam_dir: str) -> dict:
    """Read intrinsics.json from a PandaSet camera directory."""
    import json
    path = os.path.join(cam_dir, "intrinsics.json")
    with open(path) as f:
        return json.load(f)


def _load_pandaset_poses(cam_dir: str) -> list:
    """Read poses.json from a PandaSet camera directory."""
    import json
    path = os.path.join(cam_dir, "poses.json")
    with open(path) as f:
        return json.load(f)


def load_pandaset_cameras(
    base_dir: str,
    args,
    camera_name: str = "front_camera",
    scale: int = 4,
) -> tuple[dict, dict]:
    """Load PandaSet camera images and poses for calibration supervision.

    Reads data files directly from the PandaSet directory layout:
        {base_dir}/{seq}/camera/{camera_name}/{frame:02d}.jpg
        {base_dir}/{seq}/camera/{camera_name}/intrinsics.json
        {base_dir}/{seq}/camera/{camera_name}/poses.json

    The LiDAR world frame is used as the reference frame (matching the LiDAR loader).
    cam2world is taken directly from the camera pose entries (same world frame as LiDAR).

    Returns:
        cameras: dict {frame_index: Camera}
        images:  dict {frame_index: Tensor (H, W, 3) float32 in [0, 1]}
    """
    import cv2
    import json
    from lib.scene.cameras import Camera
    from lib.utils.console_utils import blue, red

    source_root = _resolve_source_root(base_dir)

    seq = str(getattr(args, "seq", getattr(args, "pandaset_seq", "001")))
    frames = args.frame_length  # [start, end]
    frame_ids = list(range(frames[0], frames[1] + 1))

    cam_dir = os.path.join(source_root, seq, "camera", camera_name)
    if not os.path.isdir(cam_dir):
        raise FileNotFoundError(
            f"PandaSet camera dir not found: {cam_dir}. "
            f"Available cameras: {PANDASET_CAMERA_NAMES}"
        )

    # Intrinsics
    intr = _load_pandaset_intrinsics(cam_dir)
    fx_orig = float(intr["fx"])
    fy_orig = float(intr["fy"])
    cx_orig = float(intr["cx"])
    cy_orig = float(intr["cy"])

    # Probe image dimensions from first image
    first_img_path = os.path.join(cam_dir, f"{frame_ids[0]:02d}.jpg")
    if not os.path.exists(first_img_path):
        # Fallback to 00.jpg
        first_img_path = os.path.join(cam_dir, "00.jpg")
    probe = cv2.imread(first_img_path)
    if probe is None:
        raise FileNotFoundError(f"Cannot probe image: {first_img_path}")
    ...
    cache_dir = os.path.join(source_root, seq, f"cache_cam_{camera_name}_s{scale}")
    ...
    for frame_id in frame_ids:
        if frame_id >= len(pose_list):
            print(red(f"[PandaSet cam] frame {frame_id} out of pose range ({len(pose_list)}), skipping"))
            continue
        ...
        if os.path.exists(cache_path):
            ...
            img_path = os.path.join(cam_dir, f"{frame_id:02d}.jpg")
            if not os.path.exists(img_path):
                print(red(f"[PandaSet cam] Image not found: {img_path}, skipping"))
                continue

            cam2world = _pose_dict_to_matrix(pose_list[frame_id])
            R = torch.from_numpy(cam2world[:3, :3].astype(np.float32))
            cam_center = cam2world[:3, 3].astype(np.float32)
            T = -R.T @ torch.from_numpy(cam_center)

            img_bgr = cv2.imread(img_path)
            if img_bgr is None:
                print(red(f"[PandaSet cam] Failed to read: {img_path}"))
                continue
    ...
    print(blue(f"[PandaSet cam] Loaded {len(cameras)} frames "
               f"({W}×{H} @ 1/{scale} scale) for seq '{seq}' camera '{camera_name}'"))
