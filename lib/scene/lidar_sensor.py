...
class LiDARSensor:
    def __init__(self, sensor2ego, name, inclination_bounds, data_type):
        ...
        elif self.data_type in {"KITTI", "PandaSet", "KITTICalib"}:
    ...
    def range2point(self, frame, range_map):
        ...
        H, W = range_map.shape
        rays_o, rays_d = self.get_range_rays(frame)
    ...
    def get_range_rays(self, frame):
        ...
        if self.azimuth_offsets is not None:
            row_offsets = torch.as_tensor(
                self.azimuth_offsets, device="cuda", dtype=torch.float32
            )
            if row_offsets.ndim != 1 or row_offsets.numel() != self.H:
                raise ValueError(
                    f"azimuth_offsets must be 1D with H={self.H} values, got {tuple(row_offsets.shape)}"
                )
            if torch.max(torch.abs(row_offsets)) > (torch.pi + 1.0e-3):
                row_offsets = torch.deg2rad(row_offsets)
            azimuth = azimuth + row_offsets.view(self.H, 1)
        ...
        if len(ir) == 2:
            ...
            ray_angles = torch.tensor(ir, device="cuda", dtype=torch.float32)
            # Some loaders may provide ring angles in degrees. Normalize to radians
            # so ray generation is unit-consistent.
            if torch.max(torch.abs(ray_angles)) > (torch.pi + 1.0e-3):
                ray_angles = torch.deg2rad(ray_angles)
            if self.data_type == "PandaSet":
                # PandaSet / Pandar64 rings are already stored top-to-bottom in the
                # same order as the range image rows.
                inclination = grid_y * ray_angles.unsqueeze(-1)
            else:
                inclination = grid_y * ray_angles.flip(0).unsqueeze(-1)
